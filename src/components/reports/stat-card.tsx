...
export function StatCard({ label, value, sub, warning, highlight }: StatCardProps) {
  return (
    ...
    >
      <p className={`text-xs font-medium uppercase tracking-wide ${highlight ? 'text-red-600' : 'text-gray-500'}`}>
        {label}
      </p>
      <p className={`mt-1 text-2xl font-bold tracking-tight tabular-nums ${highlight ? 'text-red-700' : 'text-gray-900'}`}>{value}</p>
